...
For each region we show the distribution of a school's value (as indicated by our best-value metric) with respect to whether or not it has at least median proportion of minority students. We notice a difference in regions, but on average we see that schools with higher proportion of minority students have higher values. This makes sense given how we constructed our best-value metric.
...
\caption{\textbf {Multi-Dimesional Scaling.}}
\end{figure}
...
Now, we have a much smaller, much more interpretable data set and a very accurate model. Thus, we've created a product that the NGO can easily use: they show us 10 components of a given school (seen the results section), and we'll tell them whether or not they should be targeted for minority-driven funding.
